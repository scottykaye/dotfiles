...
description: >
  ...
  `guides/`, `rfcs/`, `coverage-reports/`), and never writes docs to the active worktree's `docs/` folder.
...
> `coverage-reports/nova-pulse-case-management.md`). If the user names a project- or
> team-specific subfolder (e.g. `coverage-report-pulse/`), honor it under `$DOCS_ROOT`.
