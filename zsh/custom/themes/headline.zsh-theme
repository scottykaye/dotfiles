...
# Foreground color aliases (Scott's custom palette)
black=$'\e[30m'
red=$'\e[38;5;198m'
plum=$'\e[38;5;140m'
turquoise=$'\e[38;5;67m'
orange=$'\e[38;5;209m'
green=$'\e[38;5;191m'
yellow=$'\e[38;5;229m'
blue=$'\e[38;5;81m'
...
# Get git status
headline-git-status() {
  local parts=( ${(ps:$HL_TEMPLATE_TOKEN:)HL_CONTENT_TEMPLATE[STATUS]} )
  local style=${${parts[1]##*%\{}%%%\}*}
...
headline-precmd() {
  ...
  # Acquire contents
  local -A contents
  local -A content_lengths
  local -i content_length=0
  for key val in "${(@kv)HL_CONTENT_SOURCE}"; do
    content_lengths[$key]=0
    (( $COLUMNS < ${HL_COLS_REMOVAL[$key]:-0} )) && continue
    (( ! $HL_LAYOUT_ORDER[(Ie)$key] )) && continue
  ...
  local -A layouts
  local -A layout_lengths
  local -i layout_length=0
  local -A first_layout_lengths
  for key val in "${(@kv)HL_LAYOUT_TEMPLATE}"; do
    ...
    if [[ ${key[1]} != '_' ]]; then
      (( content_lengths[$key] <= 0 )) && continue
      (( length -= ${#HL_TEMPLATE_TOKEN} * 2 ))
      (( first_length -= ${#HL_TEMPLATE_TOKEN} * 2 ))
    fi
    layouts[$key]=$val
  ...
  for key in $HL_LAYOUT_ORDER; do
    (( ! $HL_TRUNC_ORDER[(Ie)$key] )) && continue
    (( $trunc_initial_length >= $content_lengths[$key] )) && continue
  ...
  for key in $HL_LAYOUT_ORDER; do
    [[ $key == '_PRE' ]] && continue
    (( content_lengths[$key] <= 0 && layout_lengths[$key] <= 0 )) && continue
  ...
  for key in $HL_TRUNC_ORDER; do
    ...
    (( $target_length + $layout_length - $offset <= $COLUMNS )) && break
  ...
  for key in $HL_LAYOUT_ORDER; do
    ...
    if [[ ${key[1]} == '_' && ${#contents[$key]} == 0 ]]; then
      segment="${layouts[$key]}"; segment_sep=$segment
    elif [[ ${key[1]} == '_' && ${#contents[$key]} != 0 ]]; then
  ...
  if [[ $HL_OVERWRITE == 'on' && $_HL_CMD_NUM == $_HL_CMD_NUM_PREV ]]; then
    ...
    print -nr "$cursor_up"
    (( ${#HL_OUTPUT_INFO} )) && print -nr "$cursor_up"
    (( ${#HL_OUTPUT_SEP} )) && print -nr "$cursor_up"
